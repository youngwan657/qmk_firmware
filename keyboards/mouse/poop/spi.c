...
// Mouse Implementation with PMW3360
static inline uint8_t pmw_read(const uint8_t addr, const uint8_t SSpin) {
  SS_LOW(SSpin);
  wait_us(1);
  SPI_SendByte(addr & 0x7f);
  wait_us(160);  // t_SRAD 160
  uint8_t data = SPI_ReceiveByte();
  //wait_us(1);  // tSCLK-NCS for read operation is 120ns
  SS_HIGH(SSpin);
  wait_us(19);  // subsequent write delay tSRW / tSRR(= 20us) minus tSCLK - NCS
  return data;
}
static inline void pmw_write(const uint8_t addr, const uint8_t data, uint8_t SSpin) {
  SS_LOW(SSpin);
  SPI_SendByte(addr | 0x80);  // send 1 MSB (0x80) to indicate write
  SPI_SendByte(data);
  wait_us(20);  // maximum of t_SWW, t_SWR, t_SCLK-NCS for pmw?
  SS_HIGH(SSpin);
  wait_us(160);  // t_SWW 180- tsclk-ncs
}
static void point_init(const uint8_t SSpin) {
 ...
 // uprintf("initial reset\n");
  ...
  pmw_read(Motion, SSpin);
  pmw_read(Delta_X_L, SSpin);
...
static void point_burst_read(report_mouse_t* report, bool AsMotion, const uint8_t SSpin, bool* BurstState) {
  // if no other register read is done then no need to write burst mode...
  union data16_t _a; //interchanagble x and y depending on the module ornientation
  union data16_t _b;
  if (!BurstState) {
      pmw_write(Motion_Burst, 0x01, SSpin);  // Write something to enable bust mode
      *BurstState = true;
   //   uprintf("Burst Reset\n");
  }
  SS_LOW(SSpin);
  ...
  _a.L = SPI_ReceiveByte(); // lower 8 bits of x
  _a.H = SPI_ReceiveByte(); // top 8 bits of x
  _b.L = SPI_ReceiveByte();
  _b.H = SPI_ReceiveByte();
 // uprintf("a.L: %d, a.H: %d, b.L: %d, b.H %d\n", _a.L, _a.H, _b.L, _b.H);
  ...
  // delay(); tBEXIT is 500ns, no need to wait with matrix_scan and other spi
}
static void mouse_report(uint8_t SSpin, bool bMotion, bool* bBurst) {
    report_mouse_t currentReport = pointing_device_get_report();
    point_burst_read(&currentReport, bMotion, SSpin, bBurst);
